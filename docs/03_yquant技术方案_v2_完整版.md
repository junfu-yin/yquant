...
```
┌─────────────────────────────────────────────────────────────┐
│  调度层  APScheduler（交易日 16:30 数据更新 → 17:00 盘后简报   │
│          → 17:30 推送；当晚 21:30 晚间公告补采 → 增量推送红旗；  │
│          次日 08:30 隔夜补充简报；周一 08:00 复盘周报）          │
└──────┬──────────────────────────────────────────────────────┘
       │
┌──────▼──────────┐   ┌──────────────────┐   ┌────────────────┐
...
| 需求 | 直接复用 | 禁止自研的原因 |
|---|---|---|
| 交易日历 | AkShare `tool_trade_date_hist_sina` 或 Tushare `trade_cal` 落盘 | 自己推算节假日必错 |
| 前/后复权 | AkShare 复权接口（`stock_zh_a_hist` 的 `adjust` 参数）直接取，或落盘复权因子 | 手推复权因子易在送转/配股上出错 |
| A 股 broker（T+1/涨跌停/撮合） | 参考 backtrader 中文社区 T+1 broker 实现后改造（本项目自研点，但站在参考实现上） | 从零写事件级撮合成本高 |
| 涨跌停时变规则 | 表已在 §5.1 给出，直接实现纯函数 | 已给数据，无需再调研 |
...
| 结构化输出校验 | `pydantic` v2 | — |
| 中文数字/金额归一化 | `cn2an` 等库处理"万/亿/中文数字"，再自写容差比对 | 自己写中文数字解析易漏 case |
...
class DataRepo:
    ...
    def get_bars(self, symbols: list[str], start: date, end: date,
        ...
        adj_factor,is_suspended,limit_up_price,limit_down_price,price_limit_pct
        保证: 按交易日历对齐; 停牌日 is_suspended=True 且价格前向填充并标记;
        涨跌停价由 limit_rule() 按【当日日期 + 板块 + 是否 ST + 是否新股上市初期】
        时变计算(见下方硬性说明), 而非静态百分比表。"""
...
**涨跌停规则（时变，硬性说明，M1 落盘时预计算）**：
涨跌停幅度**随日期、板块、股票状态变化**，禁止用一张静态百分比表覆盖 2016 至今全区间（否则 T3 陷阱测试形同虚设）。实现一个纯函数 `limit_rule(symbol, board, date, is_st, list_date) -> pct | None`，返回当日涨跌停幅度（`None` = 当日不设限）。至少覆盖以下时变事实（AS-6：外包在 M0 核对以下时点与幅度，落定至 `assumptions.md`；有冲突以交易所规则为准）：

| 板块 | 幅度 | 生效时点 / 例外 |
|---|---|---|
| 主板（沪/深） | ±10% | 全区间；ST/*ST 为 ±5% |
| 创业板 | ±10% → ±20% | **2020-08-24 注册制改革起**由 ±10% 改为 ±20%；此前 ±10% |
| 科创板 | ±20% | **2019-07-22 开板起**；开板前不存在 |
| 北交所 | ±30% | **2021-11-15 开市起**；此前为新三板精选层规则 |
| 新股上市初期 | 无限制 / 特殊 | 科创板、创业板（注册制后）**上市前 5 个交易日不设涨跌停**；主板新股首日 ±44%/-36% 等历史规则按上市日所属制度处理 |

`price_limit_pct` 字段随每根 K 线落盘，`limit_up_price/limit_down_price` 由前收盘价按当日 `price_limit_pct` 计算；`price_limit_pct is None`（如新股前 5 日）时两个价格字段置空并标记"当日无涨跌停约束"。

**质检器（每日更新后强制运行）**：K 线覆盖率对照交易日历（缺失即报）、价格异常跳变检测（**阈值按 `price_limit_pct` 动态设定：|Δclose| 超过 当日涨跌停幅度+1% 缓冲 且非除权除息日/非无限制日 → 报警**，不再用固定 21%）、复权因子连续性、公告时间戳非未来。产出 `QualityReport` 存表并在 UI 显示最近数据日期。
...
1. **T+1**：当日买入股份 `available_for_sell=False`，次一交易日解锁。**实现要点（难点，预先点明）**：backtrader 的 `Position` 不原生记录每笔买入的可卖日，必须自建"持仓批次账本"——每次买入记 `(shares, buy_date)`，卖出时只允许卖出 `buy_date < 当前交易日` 的批次并按 FIFO 扣减。允许参考社区 A 股 broker 实现（如 backtrader 中文社区的 T+1 broker）后改造，不必从零摸索。
2. **涨跌停撮合**：涨跌停价来自 M1 落盘的 `limit_up_price/limit_down_price`（时变，见 §5.1）。撮合规则：
   - 目标日 `open==limit_up_price`（一字涨停）→ 买单拒绝成交；`open==limit_down_price`（一字跌停）→ 卖单拒绝。
   - 非一字但**收盘封板**：`close==limit_up_price` → 视为当日封死涨停，买单拒绝成交（现实中封板买不进）；`close==limit_down_price` → 卖单拒绝。
   - 其余触板但未封死的日子：以 `min(委托价, 收盘价)`（买）/`max(委托价, 收盘价)`（卖）成交，保守化处理。
   - `price_limit_pct is None`（新股前 5 日等无限制日）不做涨跌停拒单。
...
**验收标准**：§8 陷阱测试集 T1–T6 全绿；同配置重复运行结果逐字节一致（确定性）；全 A universe 十年月频回测单次运行 ≤10 分钟。

> **性能实现路径（预先给方向，避免在优化上无止境烧 token）**：backtrader 事件驱动 + 全 A（约 5000 只）× 10 年日线，纯事件循环大概率达不到 10 分钟。**推荐分层**：①信号计算与选股（均线、动量、波动率排序等）在 backtrader **之外**用 pandas/numpy **向量化预计算**，产出每个调仓日的目标持仓列表；②backtrader 只负责按目标持仓做撮合、约束（T+1/涨跌停/停牌/成本）与净值核算，不在 Strategy 内做重指标计算；③预计算结果按 `(universe_hash, 参数, 区间)` 落盘缓存，walk-forward 各窗口复用。若采用缓存，需在模块 README 说明缓存键与失效条件。此路径为建议，允许承接方提出等效替代方案（走变更流程）。
...
**简报覆盖窗口（重要，与"零遗漏"指标直接相关）**：A 股大量重要公告（尤其减持、监管函、重大事项、业绩预告）在**当晚 18:00–23:00** 披露，17:00 的盘后简报必然覆盖不全。因此定义两班次：
- **盘后简报（17:00）**：覆盖窗口 = 上一交易日 15:00 至当日 17:00 采集到的内容，头部标注"截至 17:00，晚间公告见 21:30 补采"。
- **晚间补采（21:30）**：仅对当日新增公告增量结构化，若产生 severity≥4/红旗事件则增量推送；无则静默。
- 次日 08:30 生成"隔夜补充"合并视图，保证 §2.3 的**重大事件（severity≥4）零遗漏以"当日 21:30 班次 + 次日 08:30 合并"为口径**，而非仅以 17:00 班次为准。
- （此窗口划分为默认建议，委托人可在评审时调整时点；调整只改调度配置，不改流水线代码。）
...
**输入裁剪策略（硬性，直接决定 token 与 ¥2/日 预算能否守住）**：单份公告正文可达数万至十几万字（年报、重组报告书），逐条整篇喂 LLM 会击穿 token 预算与"单日 ≤15 分钟"约束。规定：
1. **优先规则抽取要素段**：对结构化程度高的公告（定期报告、减持、质押），先用正则/关键段落定位（如"本次减持""质押比例""归属于母公司股东的净利润"附近 ±N 字）抽取要素段，只把要素段 + 标题喂 LLM。
2. **兜底截断**：无法定位要素段时，对正文做 `MAX_INPUT_CHARS`（默认 8000 字，config 可调）**左截断保留首尾**（首部含核心结论、尾部含落款/关键数据），并在卡片标记 `input_truncated=True`。
3. **分类前置**：能用标题 + source_type 规则直接判定的低价值类目（如"关于召开股东大会的通知"）走规则分类，**不进 LLM**，直接省 token。
...
**幻觉防护（硬性规则，代码强制）**：
- **数字回验采用"归一化后比对"，非朴素正则**（这是本模块核心攻坚点，值得投入）：原文数字形态多样（`1,000万` / `1000万` / `0.1亿` / `10,000,000` / `10.00%`）。回验流程 = ①把 `key_numbers` 与原文候选数字都归一化到统一量纲（去千分位、中文数量单位"万/亿"折算、百分号处理），②在容差内（默认相对误差 1e-6）比对。**只有归一化后仍无法在原文找到匹配的数字，才判定为编造**。目标：编造数字漏杀率 = 0 的同时，避免把正确卡片大量误杀为 `unverified`（后者会拖垮分类/方向准确率）。
...
**验收标准**：交付评测集（外包整理素材、委托人终审标注；覆盖 12 类 event_type、正负样本均衡），**样本量 ≥120 条（每类 ≥8 条，严重事件类目——减持/质押/监管处分/诉讼——每类 ≥12 条）**，以降低小样本统计噪声。指标：
- event_type 准确率 ≥85%；
...
> 口径说明：§2.3 的"severity≥4 遗漏率 = 0"是**产品级目标**（配合 21:30/次日 08:30 补采班次达成），本处的"召回率 ≥95%"是**单模型可验收的工程指标**，二者不再冲突。
...
APScheduler 进程常驻（`yquant daemon` 命令）：交易日 16:30 `update_all` → 成功后 17:00 盘后简报流水线 → 17:30 推送；**当晚 21:30 晚间公告补采 + 增量结构化（有 severity≥4/红旗才增量推送）；次日 08:30 生成隔夜补充合并视图**；周一 08:00 周报。交易日历判断用本地日历表。所有任务写 `job_runs` 表（开始/结束/状态/错误摘要），UI 设置页可见最近 20 次。日志用 loguru，按天轮转，保留 30 天。
...
├── yquant/
│   ├── datasrc/               # M1: sources/(akshare_,tushare_,baostock_), repo.py, quality.py, limit_rule.py
...
- `data/parquet/daily_bars/`（分区键：`year`；列含 §5.1 `get_bars` 全部字段，含 `price_limit_pct/limit_up_price/limit_down_price/is_suspended`）。
- `data/parquet/index_bars/`（分区键：`index_code`）。
- `data/parquet/financials/`（分区键：`report_period`，即报告期 YYYYMM；**每行必带 `announce_date`（真实或推算）与 `announce_date_estimated` 标记**，这是 T1 前视偏差防护的落盘依据——回测按 `回测日 >= announce_date` 决定该行是否可见）。
...
event_cards(id PK, symbol, date, source_type, event_type, severity, direction,
            ...
            -- batch: 简报班次(afterhours_17:00 / night_21:30 / overnight_0830)
...
| T3b 涨跌停时变 | 构造 2019 与 2021 两个日期的创业板股票+触板信号（跨越 2020-08-24 规则变更点） | 对比两次撮合用的涨跌停幅度 | 2019 用 ±10%、2021 用 ±20%；证明 `limit_rule` 按日期时变而非静态表 |
...
| T7 封板买不进 | 构造非一字但收盘=涨停价的日子+当日买入信号 | 成交记录 | 买单拒绝成交（对齐 §5.2 收盘封板规则） |
...
|---|---|---|---|
| M0 环境与假设验证 | 仓库/CI/依赖；AS-1~6 接口与规则实测报告 | 3–4 | assumptions.md 全部落定（含 AS-6 涨跌停时点核对）；三数据源冒烟通过 |
...
**Q5 历史公告拿不到全量正文怎么办？** 允许"标题+链接"降级模式，EventCard 打 `unverified`；正文可得性以 M0 实测为准（AS-1）。长正文的截断策略见 §5.4 输入裁剪策略。
